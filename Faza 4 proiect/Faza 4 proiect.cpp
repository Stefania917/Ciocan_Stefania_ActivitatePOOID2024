...
int main() {
    int nrSmartphones, nrCalculatoare, nrCeasuri;
    cout << "Introduceti numarul de smartphone-uri: ";
    cin >> nrSmartphones;
    Smartphone* smartphones = new Smartphone[nrSmartphones];

    // Citirea smartphone-urilor
    for (int i = 0; i < nrSmartphones; i++) {
        string marca;
        int nrPreturi;
        cout << "Introduceti marca smartphone-ului " << i + 1 << ": ";
        cin >> marca;
        cout << "Introduceti numarul de preturi: ";
        cin >> nrPreturi;
        float* preturi = new float[nrPreturi];
        cout << "Introduceti preturile: ";
        for (int j = 0; j < nrPreturi; j++) {
            cin >> preturi[j];
        }
        smartphones[i] = Smartphone(marca, nrPreturi, preturi);
        delete[] preturi;
    }

    // Afisarea smartphone-urilor
    for (int i = 0; i < nrSmartphones; i++) {
        smartphones[i].afisare();
    }

    // Citirea calculatoarelor
    cout << "Introduceti numarul de calculatoare: ";
    cin >> nrCalculatoare;
    Calculator* calculatoare = new Calculator[nrCalculatoare];

    for (int i = 0; i < nrCalculatoare; i++) {
        string model;
        int nrSpecificatii;
        cout << "Introduceti modelul calculatorului " << i + 1 << ": ";
        cin >> model;
        cout << "Introduceti numarul de specificatii: ";
        cin >> nrSpecificatii;
        int* specificatii = new int[nrSpecificatii];
        cout << "Introduceti specificatiile: ";
        for (int j = 0; j < nrSpecificatii; j++) {
            cin >> specificatii[j];
        }
        calculatoare[i] = Calculator(model, nrSpecificatii, specificatii);
        delete[] specificatii;
    }

    for (int i = 0; i < nrCalculatoare; i++) {
        calculatoare[i].afisare();
    }

    cout << "Introduceti numarul de ceasuri inteligente: ";
    cin >> nrCeasuri;
    CeasInteligent* ceasuri = new CeasInteligent[nrCeasuri];

    for (int i = 0; i < nrCeasuri; i++) {
        string brand;
        int nrFunctii;
        cout << "Introduceti brandul ceasului inteligent " << i + 1 << ": ";
        cin >> brand;
        cout << "Introduceti numarul de functii: ";
        cin >> nrFunctii;
        bool* functii = new bool[nrFunctii];
        cout << "Introduceti functiile: ";
        for (int j = 0; j < nrFunctii; j++) {
            cin >> functii[j];
        }
        ceasuri[i] = CeasInteligent(brand, nrFunctii, functii);
        delete[] functii;
    }
